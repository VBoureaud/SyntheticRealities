...
function Home() {
  ...
  return (
    <>
      ...
      <div className="inner-shadow" style={{ maxWidth: '940px', margin: 'auto' }}>
        {!store.party && <div className="header">
          <h1 style={{ fontWeight: 'normal', color: 'white', letterSpacing: '1px' }}>Who Made This?</h1>
          {
            store.playerName && <div style={{ display: 'flex', justifyContent: 'center' }}>
              <p style={{ marginTop: '5px', marginBottom: '5px' }} className="white">Connected as <strong>{store.playerName}</strong></p>
              <EditOutlined onClick={() => setIsModalOpen(true)} style={{ color: 'white', marginLeft: '5px', cursor: 'pointer' }} />
            </div>
          }
        </div>}
